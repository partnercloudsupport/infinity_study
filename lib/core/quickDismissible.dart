...
/// Signature used by [QuickDismissible] to indicate that it has been dismissed in
/// the given `direction`.
///
/// Used by [QuickDismissible.onDismissed].
typedef void DismissDirectionCallback(DismissDirection direction);

/// The direction in which a [QuickDismissible] can be dismissed.
enum QuickDismissDirection {
  /// The [QuickDismissible] can be dismissed by dragging either up or down.
  vertical,

  /// The [QuickDismissible] can be dismissed by dragging either left or right.
  horizontal,

  /// The [QuickDismissible] can be dismissed by dragging in the reverse of the
  /// reading direction (e.g., from right to left in left-to-right languages).
  endToStart,

  /// The [QuickDismissible] can be dismissed by dragging in the reading direction
  /// (e.g., from left to right in left-to-right languages).
  startToEnd,

  /// The [QuickDismissible] can be dismissed by dragging up only.
  up,

  /// The [QuickDismissible] can be dismissed by dragging down only.
  down,
...
class QuickDismissible extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument must not be null because [QuickDismissible]s are commonly
  /// used in lists and removed from the list when dismissed. Without keys, the
  /// default behavior is to sync widgets based on their index in the list,
  /// which means the item after the dismissed item would be synced with the
  /// state of the dismissed item. Using keys causes the widgets to sync
  /// according to their keys and avoids this pitfall.
  const QuickDismissible({
    @required Key key,
    @required this.child,
    this.background,
    ...
    this.resizeDuration: const Duration(milliseconds: 1),
    this.dismissThresholds: const <DismissDirection, double>{},
    this.movementDuration: const Duration(milliseconds: 100),
...
class _DismissibleState extends State<QuickDismissible> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin { // ignore: MIXIN_INFERENCE_INCONSISTENT_MATCHING_CLASSES
  @override
  void initState() {
    super.initState();
